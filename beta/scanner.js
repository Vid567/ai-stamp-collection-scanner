...
function writePhotoSequence() {
  sessionStorage.setItem(PHOTO_SEQUENCE_KEY, String(nextPhotoSequence));
}

function photoSequenceFromLabel(label = "") {
  return Number.parseInt(String(label).replace(/\D/g, ""), 10) || 0;
}

function syncPhotoSequenceFromRestoredPhotos() {
  const highestExisting = Math.max(0, ...photos.map(photo => photoSequenceFromLabel(photo.number)));
  nextPhotoSequence = Math.max(nextPhotoSequence, highestExisting + 1);
  writePhotoSequence();
}

function takeNextPhotoNumber() {
  const number = photoNumber(nextPhotoSequence - 1);
  nextPhotoSequence += 1;
  writePhotoSequence();
  return number;
}
...
async function restoreState() {
  try {
    ...
    syncPhotoSequenceFromRestoredPhotos();
    render();
...
async function addFiles(fileList) {
  ...
  for (const file of accepted) {
    const id = newId();
    const photo = {id, number: takeNextPhotoNumber(), name: file.name, type: file.type, size: file.size, blob: file};
    photos.push(photo);
...
$("#clear-all").onclick = async () => {
  if (!photos.length || confirm("Clear all photos and inventory records from this browser? Photo numbering will continue in this browser session.")) {
    photos = []; records = []; localStorage.removeItem(STORAGE_KEY);
    let recoveryCleared = true;
    try { await removeStoredPhotos(); } catch { recoveryCleared = false; }
    render();
    const nextLabel = photoNumber(nextPhotoSequence - 1);
